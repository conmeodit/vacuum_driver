...
class ExplorationNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('safety_zone_extra_m', 0.08)
        self.declare_parameter('safety_zone_radius_m', -1.0)
        self.declare_parameter('publish_debug_markers', True)
        self.declare_parameter('debug_marker_topic', '/exploration/debug_marker')
        ...
        self.safety_zone_extra_m = max(
            0.0, float(self.get_parameter('safety_zone_extra_m').value)
        )
        configured_safety_zone_radius = float(
            self.get_parameter('safety_zone_radius_m').value
        )
        ...
        min_footprint_radius = self.robot_radius + self.collision_margin
        if configured_safety_zone_radius > 0.0:
            self.safety_zone_radius_m = max(
                configured_safety_zone_radius,
                min_footprint_radius,
            )
        else:
            self.safety_zone_radius_m = min_footprint_radius + self.safety_zone_extra_m
        ...
        self.debug_marker_pub = self.create_publisher(Marker, self.debug_marker_topic, 10)
    ...
    def _front_block_distance(self) -> float:
        return max(self.front_stop_distance, self.safety_zone_radius_m + 0.08)

    def _align_front_block_distance(self) -> float:
        return max(self.align_front_block_distance, self.safety_zone_radius_m + 0.04)

    def _side_block_distance(self) -> float:
        return max(self.side_clearance, self.safety_zone_radius_m + 0.02)

    def _publish_safety_zone_marker(self):
        if not self.publish_debug_markers or not self.have_pose:
            return

        marker = Marker()
        marker.header.frame_id = self.odom_frame
        marker.header.stamp = self.get_clock().now().to_msg()
        marker.ns = 'exploration_debug'
        marker.id = 0
        ...
        marker.scale.x = 2.0 * self.safety_zone_radius_m
        marker.scale.y = 2.0 * self.safety_zone_radius_m
        marker.scale.z = 0.02
        marker.color.r = 0.0
        marker.color.g = 0.8
        marker.color.b = 1.0
    ...
    def _obstacle_near(self) -> bool:
        ...
        front_threshold = self._front_block_distance()
        side_threshold = self._side_block_distance()
        return (
            self.sector_min['front'] < front_threshold
            or self.sector_p['front_left'] < side_threshold
            or self.sector_p['front_right'] < side_threshold
        )
    ...
    def _obstacle_cleared(self) -> bool:
        ...
        side_threshold = self._side_block_distance()
        return (
            self.sector_p['front'] > (front_threshold + 0.12)
            and self.sector_p['front_left'] > (side_threshold + 0.08)
            and self.sector_p['front_right'] > (side_threshold + 0.08)
        )
